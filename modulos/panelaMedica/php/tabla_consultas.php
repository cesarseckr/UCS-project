<div id="tabla" class="table-responsive" style="padding-bottom: 15px;">
  ...
  <table id="tabla-1" class="table table-bordered" cellspacing="0" width="100%">
    ...
    <tbody>
      <? 
        $sql="SELECT * FROM consultas WHERE fecha = '$fecha' and dirigir = '1'";
        $sq= $db->query($sql);
        $rows=$sq->fetchAll();
        foreach ($rows as $row) {
         ...
         if ($estatus=='1') {
           ...
           $tomar='<button id="btn_diagnostico" id_consulta="'.$id_consulta.'" 
             ...
             data-whatever="@mdo"><i class="mdi
             mdi-account-plus"></i>&nbsp;
             Tomar</button>';
           $ver='<a class="btn btn-secondary disabled"
           id="ver_archivos" 
           id_consulta="'.$id_consulta.'
            href="" target="_blank"><i class="fas fa-print"></i></a>';
         }elseif ($estatus=='2') {
           ...
           $tomar='<button type="button" class="btn btn-dark" data-toggle="modal" data-target="#add_diagnostico" data-whatever="@mdo" disabled><i class="mdi mdi-account-plus" id_consulta="'.$id_consulta.'"
            talla="'.$talla.'"
             peso="'.$peso.'"
             imc="'.$imc.'"
             presion_arterial="'.$presion_arterial.'"
           ...
           id="ver_archivos" 
           id_consulta="'.$id_consulta.'"
            href="pdf/receta_medica.php?id_consulta='.$id_consulta.'" target="_blank"><i class="fas fa-print"></i></a>';
         }elseif ($estatus=='3') {
