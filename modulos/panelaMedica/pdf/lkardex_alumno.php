...
$fecha_fin=$_POST['fecha_fin_f'];
  ...
  foreach ($rows as $row){
    ...
    $nombre=$row["nombre"];
    $count=0;
    $sql_consulta="SELECT * FROM consultas WHERE fecha BETWEEN '$fecha_inicio' AND '$fecha_fin'";
    $sq_consulta=$db->query($sql_consulta);
    $rows_consulta=$sq_consulta->fetchAll();
    foreach ($rows_consulta as $row_consulta){
