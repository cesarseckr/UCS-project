...
$fecha_fin=$_POST['fecha_fin_f'];
  ...
  $filename = 'Reporte de Morbilidad (Incidencia por Diagnóstico)';

  $tabla_prueba="";

  $sql="SELECT lista_diagnosticos.nombre , COUNT(diagnostico.id_diagnostico) as conta FROM consultas INNER JOIN diagnostico ON consultas.id_consulta=diagnostico.id_consulta AND consultas.fecha BETWEEN '$fecha_inicio' and '$fecha_fin' INNER JOIN lista_diagnosticos ON lista_diagnosticos.id_lista_diagnosticos=diagnostico.diagnostico GROUP BY diagnostico.diagnostico";
  $sq= $db->query($sql);
  $rows=$sq->fetchAll();
  foreach ($rows as $row){
