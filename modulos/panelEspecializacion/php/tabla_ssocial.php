...
<div id="tabla" class="table-responsive" style="padding-bottom: 15px;">
  ...
  <table id="tabla-1" class="table table-bordered" cellspacing="0" width="100%">
    <thead>
      <tr>
        <th class="noExport">Servicio Social</th>
        <th>Nombre</th>
        <th>Matricula</th>
        <th>Curp</th>
        <th>Grupo</th>
...
$sql="SELECT * FROM municipios where id_municipio='$municipio'";
      ...
      foreach ($rows as $rowm) {
        $municipio_t=strtoupper($rowm["municipio"]);
          ...
          $servicio='<button class="btn btn-dark"
            id="ssocial" 
            id_alumno="'.$id_alumno.'"
            nombre="'.$nombre.'"
            matricula="'.$matricula.'"
            ...
            estatus="'.$estatus.'"
            data-toggle="modal"
            data-target="#add_ssocial"
            data-whatever="@mdo"> <i class="fa fa-paint-roller"></i>Servicio Social</button>';

          echo "
            <tr>
            <td><center>".$servicio."</center></td>
            <td>".$nombre."</td>
            <td>".$matricula."</td>
            <td>".$curp."</td>
            <td>".$grupo_nombre."</td>
