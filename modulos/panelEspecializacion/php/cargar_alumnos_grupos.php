<?php session_start();
  ...
  $sql="SELECT * FROM alumnos WHERE id_grupo=$id_grupo ORDER BY apaterno ASC";
      ...
      $alu='<input type="text" value="'.$id_alumno.'" name="alu-'.$i.'" id="alu-'.$i.'" style="display:none;"><input type="text" class="form-control" value="'.$alumno.'" readonly>';
      if($estatus==2){
      $incluir='<input type="checkbox" checked class="check" name="alu_check-'.$i.'" id="alu_check-'.$i.'"><label for="alu_check-'.$i.'" class="check"></label>';
      }
      else{
      $incluir='<input type="checkbox" class="check" name="alu_check-'.$i.'" id="alu_check-'.$i.'"><label for="alu_check-'.$i.'" class="check"></label>'; 
      }
