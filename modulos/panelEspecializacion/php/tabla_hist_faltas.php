...
<div id="tabla" class="table-responsive" style="padding-bottom: 15px;">
  ...
  <table id="tabla-1" class="table table-bordered">
    <thead>
      <tr>
        <th class="noExport">Historial</th>
        <th>Nombre</th>
        <th>Matricula</th>
        <th>Curp</th>
        <th>Grupo</th>
...
$sql="SELECT * FROM municipios where id_municipio='$municipio'";
      ...
      foreach ($rows as $rowm) {
        $municipio_t=strtoupper($rowm["municipio"]);
          ...
          $historial='<button class="btn btn-dark"
            id="historial_discip" 
            id_alumno="'.$id_alumno.'"
            nombre="'.$nombre.'"
            matricula="'.$matricula.'"
            ...
            estatus="'.$estatus.'"
            data-toggle="modal"
            data-target="#modal_historial"
            data-whatever="@mdo"> <i class="fa fa-user-times"></i> Faltas</button>';

          echo "
            <tr>
            <td><center>".$historial."</center></td>
            <td>".$nombre."</td>
            <td>".$matricula."</td>
            <td>".$curp."</td>
            <td>".$grupo_nombre."</td>
